function koreBotChat() {
    ...
    var _eventQueue = {};
    var carouselEles = [];
    var prevRange, accessToken, koreAPIUrl, fileToken, fileUploaderCounter = 0, _removeAttachments = '', globalRecState, bearerToken = '', assertionToken = '', messagesQueue = [], historyLoading = false;
    ...
    window.convertMDtoHTML = helpers.convertMDtoHTML;
    function isEven(n) {
        n = Number(n);
        return n === 0 || !!(n && !(n % 2));
    }
    ...
    chatWindow.prototype.init = function () {
        ...
        if(isTTSEnabled && (isSpeechEnabled || allowGoogleSpeech) && autoEnableSpeechAndTTS) {
            isTTSOn = true;
            setTimeout(function(){
                removeClass(document.getElementsByClassName('ttspeakerDiv')[0],"ttsOff");
            },350);
        }
    ...
    chatWindow.prototype.bindEvents = function () {
        ...
        _chatInputBox.addEventListener('keydown', function (event) {
            ...
            if (event.keyCode === 13) {
                ...
                if (document.getElementsByClassName('recordingMicrophone')[0].style.display === "") {
                    document.getElementsByClassName('recordingMicrophone')[0].click();
                }
        ...
        bot.on("open", function (response) {
            ...
            if(!loadHistory) {
                setTimeout(function(){
                    ...
                    removeClass(document.getElementsByClassName('disableFooter')[0],"disableFooter");
                });
            }
            _chatInputBox.focus();
        });
    ...
    this.chatHistory = function(res){
        if(loadHistory) {
            ...
            if(res && res[1] && res[1].messages.length > 0) {
                ...
                document.getElementsByClassName('historyLoadingDiv').className += 'showMsg';
                    ...
                    setTimeout(function(){
                        ...
                        if(index === res[1].messages.length-1) {
                            setTimeout(function(){
                                ...
                                // document.getElementsByClassName('.chat-container').animate({
                                //     scrollTop: document.getElementsByClassName('.chat-container').prop("scrollHeight")
                                // }, 2500);
                                removeClass(document.getElementsByClassName('historyLoadingDiv')[0],"showMsg");
                                document.getElementsByClassName('chat-container')[0].insertAdjacentHTML('beforeend',"<div class='endChatContainer'><span class='endChatContainerText'>End of chat history</span></div>");
    ...
    function createSocket() {
        ...
        _connection.onclose = function (e) {
            if(document.getElementsByClassName('chatInputBox')[0].innerText !== '' && autoEnableSpeechAndTTS) {
                var me  = window.chatContainerConfig;
                me.sendMessage(document.getElementsByClassName('chatInputBox')[0]);
            }
            isRecordingStarted = false;
                ...
                ttsAudioSource.addEventListener('ended', function(){
                    setTimeout(function(){
                        if(isTTSOn && autoEnableSpeechAndTTS) {
                            document.getElementsByClassName('notRecordingMicrophone')[0].click();
                        }
                    },350);
                })
            } catch (e) {
